...
There is a few version of the memory index we are testing here:
...
Here, we do find a few correlations that are near significance. More specifically, PAM units (according to Jip: the key PAM activity marker) seem to be negatively correlated with MI and with corrected MI. This means that higher PAM activity is associated with lower memory performance. This is in line with the literature, as PAM is often associated with vascular health and inflammation, which are known to negatively affect cognitive function. In addition, pam level seems to be negatively correlated with the corrected MI.
...
pam$group_contrast <- ifelse(pam$group == "donor", 1, -1)
# raw memory index:
...
The raw MI is significant, the corrected MI is not.
